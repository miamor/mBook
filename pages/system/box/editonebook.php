...
$box->book_coverIMG = $box->book_backIMG = null;
...
if ($box->book_title /*&& $box->book_cover && $box->book_back*/ && $box->book_barcode && $box->book_square_id /*&& $box->book_price && $box->book_price_rent && $box->book_num*/) {
	if ($box->book_cover['size']) $box->book_coverIMG = $box->upload($box->book_cover);
	if ($box->book_back['size']) $box->book_backIMG = $box->upload($box->book_back, false);
//	if ($box->book_coverIMG && $box->book_backIMG) {
		$editBook = $box->editBook();
		if ($editBook) {
//			header('Content-Type: text/html; charset=utf-8');
//			echo '<script>window.location.href = "'.$box->link.'"</script>';
			echo '[type]success[/type][dataID]'.$box->link.'?b='.$box->book_id.'[/dataID][content]Sửa thành công. Đang chuyển trang...[/content]';
