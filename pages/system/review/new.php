...
if ( (!$toFB || ($toFB && $thumb) ) && $content && $rate) {
	...
	if (isset($theRv['id']) && $theRv['id']) echo '[type]error[/type][content]Duplicated content found![/content]';
		...
		if ($content && ($iid || $review->bookTitle)) {
//			echo $content.'~'.$iid.'~'.$review->bookTitle;
		...
		} else echo '[type]error[/type][content]Missing parameters!'.$review->bookTitle.'~'.$content.'~'.$rate.'[/content]';
	}
