...
function rate (element) {
	...
	$(element).find(".rating-icons").not(".rated, .myrate").children(".rating-star-icon").hover(function() {
		...
		for (i = a+1; i <= 5; i++) {
			console.log($(element).find(".rating-star-icon.v" + i));
			$(element).find(".rating-star-icon.v" + i).removeClass('fa-star').addClass('fa-star-o');
		}
		for (i = 1; i <= a; i++) {
			$(element).find(".rating-star-icon.v" + i).removeClass('fa-star-o').addClass('fa-star');
		}
	}).mouseout(function() {
		//rateVal = parseInt($(element).find('.rate-val').val());
